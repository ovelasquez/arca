 ...
 * Time: 4:37 PM
 */
class RevSliderOutput extends RevSliderElementsBase
{
...
//                $htmlBeforeSlider .= '<script type="text/javascript" src="'.$urlIncludeJS.'"></script>';
            ...
            if (!empty($backgroundColor)) {
                $containerStyle .= 'background:' . RevSliderTPColorpicker::get($backgroundColor) . ';';
            }
            ...
            if ($sliderType != 'fullscreen') {
                ...
                if ($sliderPosition != 'center') {
                    $containerStyle .= 'margin-left:' . RevSliderFunctions::esc_attr($this->slider->getParam('margin_left', '0', RevSliderSlider::FORCE_NUMERIC)) . 'px;';
                    $containerStyle .= 'margin-right:' . RevSliderFunctions::esc_attr($this->slider->getParam('margin_right', '0', RevSliderSlider::FORCE_NUMERIC)) . 'px;';
                }

                $containerStyle .= 'margin-top:' . RevSliderFunctions::esc_attr($this->slider->getParam('margin_top', '0', RevSliderSlider::FORCE_NUMERIC)) . 'px;';
                $containerStyle .= 'margin-bottom:' . RevSliderFunctions::esc_attr($this->slider->getParam('margin_bottom', '0', RevSliderSlider::FORCE_NUMERIC)) . 'px;';
            }
            ...
            $progress_style = ' style="height: ' . RevSliderFunctions::esc_attr($progress_height) . 'px; background: ' . RevSliderTPColorpicker::convert($progressbar_color, $progress_opa) . ';"';
    ...
    public function get_responsive_size()
    {
        ...
        if ($adv_resp_sizes == true) {
            $width = $this->slider->getParam("width", 1240, RevSliderSlider::FORCE_NUMERIC);
            $def = $width;
            $width .= ',';
            if ($enable_custom_size_notebook == 'on') {
                $width .= $this->slider->getParam("width_notebook", 1024, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_notebook", 1024, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $width .= $def;
            }
            $width .= ',';
            if ($enable_custom_size_tablet == 'on') {
                $width .= $this->slider->getParam("width_tablet", 778, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_tablet", 778, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $width .= $def;
            }
            $width .= ',';
            if ($enable_custom_size_iphone == 'on') {
                $width .= $this->slider->getParam("width_mobile", 480, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("width_mobile", 480, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $width .= $def;
            }

            $height = $this->slider->getParam("height", 1240, RevSliderSlider::FORCE_NUMERIC);
            $def = $height;
            $height .= ',';
            if ($enable_custom_size_notebook == 'on') {
                $height .= $this->slider->getParam("height_notebook", 1024, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_notebook", 1024, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $height .= $def;
            }
            $height .= ',';
            if ($enable_custom_size_tablet == 'on') {
                $height .= $this->slider->getParam("height_tablet", 778, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_tablet", 778, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $height .= $def;
            }
            $height .= ',';
            if ($enable_custom_size_iphone == 'on') {
                $height .= $this->slider->getParam("height_mobile", 480, RevSliderSlider::FORCE_NUMERIC);
                $def = $this->slider->getParam("height_mobile", 480, RevSliderSlider::FORCE_NUMERIC);
            } else {
                $height .= $def;
            }
            ...
            return array(
                ...
                'height'          => $this->slider->getParam("height", "868", RevSliderSlider::FORCE_NUMERIC),
                'width'           => $this->slider->getParam("width", "1240", RevSliderSlider::FORCE_NUMERIC)
            );
        }
    }
...
//        $do_delay = apply_filters('revslider_add_js_delay', $do_delay);
        ...
        <script type="text/javascript">
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                echo '			delay:' . RevSliderFunctions::esc_attr($this->slider->getParam("delay", "9000", RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                ...
                if ($slider_type !== 'hero' && ($enable_arrows == 'on' || $enable_bullets == 'on' || $enable_tabs == 'on' || $enable_thumbnails == 'on' || $touch_enabled == 'on' || $keyboard_enabled == 'on' || $mousescroll_enabled != 'off')) {
                    ...
                    if ($enable_arrows == 'on') {
                        ...
                        if ($hide_arrows_on_mobile === 'true') {
                            echo '					hide_under:' . RevSliderFunctions::esc_attr($this->slider->getParam('arrows_under_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        if ($hide_arrows_over === 'true') {
                            echo '					hide_over:' . RevSliderFunctions::esc_attr($this->slider->getParam('arrows_over_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        echo '					hide_onleave:' . $arrows_always_on . ',' . "\n";
                        if ($arrows_always_on === 'true') {
                            echo '					hide_delay:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_arrows', '200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                            echo '					hide_delay_mobile:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_arrows_mobile', '1200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        echo '					tmp:\'';
                        echo preg_replace("/\r|\n/", "", $arr_tmp);
                        echo '\',' . "\n";
                        ...
                        echo '						h_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_offset_hor', '20', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '						v_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('leftarrow_offset_vert', '0', RevSliderSlider::FORCE_NUMERIC)) . "\n";
                        echo '					},' . "\n";
                        echo '					right: {' . "\n";
                        ...
                        echo '						h_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_offset_hor', '20', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '						v_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('rightarrow_offset_vert', '0', RevSliderSlider::FORCE_NUMERIC)) . "\n";
                        echo '					}' . "\n";
                        echo '				}' . "\n";
                    }
                    ...
                    if ($enable_bullets == 'on') {
                        ...
                        if ($hide_bullets_on_mobile === 'true') {
                            echo '					hide_under:' . RevSliderFunctions::esc_attr($this->slider->getParam('bullets_under_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        if ($hide_bullets_over === 'true') {
                            echo '					hide_over:' . RevSliderFunctions::esc_attr($this->slider->getParam('bullets_over_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        ...
                        if ($bullets_always_on === 'true') {
                            echo '					hide_delay:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_bullets', '200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                            echo '					hide_delay_mobile:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_bullets_mobile', '1200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        ...
                        echo '					h_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('bullets_offset_hor', '20', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					v_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('bullets_offset_vert', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					space:' . RevSliderFunctions::esc_attr($this->slider->getParam('bullets_space', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					tmp:\'';
                        echo preg_replace("/\r|\n/", "", $bul_tmp);
                        echo '\'' . "\n";
                        echo '				}' . "\n";
                    ...
                    if ($enable_thumbnails == 'on') {
                        ...
                        echo '					width:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumb_width', '100', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					height:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumb_height', '50', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					min_width:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumb_width_min', '100', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					wrapper_padding:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_padding', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					wrapper_color:"' . RevSliderTPColorpicker::convert(RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_wrapper_color', 'transparent')), $this->slider->getParam('thumbnails_wrapper_opacity', false)) . '",' . "\n";
                        echo '					tmp:\'';
                        echo preg_replace("/\r|\n/", "", $thumbs_tmp);
                        echo '\',' . "\n";
                        echo '					visibleAmount:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumb_amount', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					hide_onmobile:' . $hide_thumbs_on_mobile . ',' . "\n";
                        if ($hide_thumbs_on_mobile === 'true') {
                            echo '					hide_under:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbs_under_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        if ($hide_thumbs_over === 'true') {
                            echo '					hide_over:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbs_over_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        echo '					hide_onleave:' . $thumbs_always_on . ',' . "\n";
                        if ($thumbs_always_on === 'true') {
                            echo '					hide_delay:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_thumbs', '200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                            echo '					hide_delay_mobile:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_thumbs_mobile', '1200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        ...
                        echo '					space:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_space', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        ...
                        echo '					h_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_offset_hor', '20', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					v_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('thumbnails_offset_vert', '0', RevSliderSlider::FORCE_NUMERIC)) . "\n";
                        echo '				}' . "\n";
                    }
                    if ($enable_tabs == 'on') {
                        ...
                        echo '					width:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_width', '100', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					height:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_height', '50', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					min_width:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_width_min', '100', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					wrapper_padding:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_padding', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					wrapper_color:"' . RevSliderTPColorpicker::convert(RevSliderFunctions::esc_attr($this->slider->getParam('tabs_wrapper_color', 'transparent')), $this->slider->getParam('tabs_wrapper_opacity', false)) . '",' . "\n";
                        echo '					tmp:\'';
                        echo preg_replace("/\r|\n/", "", $tabs_tmp);
                        echo '\',' . "\n";
                        echo '					visibleAmount: ' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_amount', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					hide_onmobile: ' . $hide_tabs_on_mobile . ',' . "\n";
                        if ($hide_tabs_on_mobile === 'true') {
                            echo '					hide_under:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_under_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        if ($hide_tabs_over === 'true') {
                            echo '					hide_over:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_over_hidden', '0', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        echo '					hide_onleave:' . $tabs_always_on . ',' . "\n";
                        if ($tabs_always_on === 'true') {
                            echo '					hide_delay:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs', '200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                            echo '					hide_delay_mobile:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs_mobile', '1200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        }
                        echo '					hide_delay:' . RevSliderFunctions::esc_attr($this->slider->getParam('hide_tabs', '200', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        ...
                        echo '					space:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_space', '5', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        ...
                        echo '					h_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_offset_hor', '20', RevSliderSlider::FORCE_NUMERIC)) . ',' . "\n";
                        echo '					v_offset:' . RevSliderFunctions::esc_attr($this->slider->getParam('tabs_offset_vert', '0', RevSliderSlider::FORCE_NUMERIC)) . "\n";
                        echo '				}' . "\n";
                    }
                    echo '			},' . "\n";
...
//            }
    ...
    private function putSlides($gal_ids = array(), $order = array())
    {
        ...
        if ($this->previewMode == true && $this->oneSlideMode == true) {
            ...
            $previewSlide = new RevSliderSlide();
...
//                        $link = do_shortcode($slide->getParam('link', ''));
            ...
            switch ($bgType) {
                ...
                case 'solid':
                    $urlSlideImage = $urlImageTransparent;
                    $slideBGColor = RevSliderTPColorpicker::get($slide->getParam('slide_bg_color', '#d0d0d0'));
...
//        $layer = apply_filters('revslider_putLayer_pre', $layer, $this, $row_group_uid, $static_slide, $special_type);
        ...
        if ($do_remove_inline) {
            ...
            if (isset($this->in_class_usage[trim($class)]) && isset($this->in_class_usage[trim($class)]['params'])) {//defaults get set here
                ...
                $dco = $dco !== false ? $dcot !== false ? $dco = RevSliderTPColorpicker::convert($dco, $dcot) : $dco = RevSliderTPColorpicker::get($dco) : $dco;
                ...
                $dbc = $dbc !== false ? $dbt !== false ? $dbc = RevSliderTPColorpicker::convert($dbc, $dbt) : $dbc = RevSliderTPColorpicker::get($dbc) : $dbc;
                ...
                $dboc = $dboc !== false ? $dbot !== false ? $dboc = RevSliderTPColorpicker::convert($dboc, $dbot) : $dboc = RevSliderTPColorpicker::get($dboc) : $dboc;
        ...
        if (RevSliderFunctions::getVal($frame_start, 'use_text_c') == 1)
            $frame_start['color'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_start, 'text_c', 'transparent'));

        if (RevSliderFunctions::getVal($frame_start, 'use_bg_c') == 1)
            $frame_start['bgcolor'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_start, 'bg_c', 'transparent'));
        ...
        $frame_start['sfxcolor'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_start, 'sfxcolor', RevSliderFunctions::getVal($layer, 'sfxcolor', '#ffffff'))); //fallback
            ...
            case 'svg':
                ...
                if (!empty($static_styles)) {
                    ...
                    if (!empty($static_styles['color'])) {
                        ...
                        $use_color = $use_color !== false ? $color_trans !== false ? $use_color = RevSliderTPColorpicker::convert($use_color, $color_trans) : $use_color = RevSliderTPColorpicker::get($use_color) : $use_color;
...
//                $html_toggle = do_shortcode(stripslashes($html_toggle));
                ...
                if (!empty($static_styles)) {
                    ...
                    if (!empty($static_styles['color'])) {
                        ...
                        $def_val = (array)RevSliderFunctions::getVal($layer, 'deformation', array());
                        $color_trans = RevSliderFunctions::getVal($def_val, 'color-transparency', false);
                        $use_color = $use_color !== false ? $color_trans !== false ? RevSliderTPColorpicker::convert($use_color, $color_trans) : RevSliderTPColorpicker::get($use_color) : $use_color;
        ...
        if (RevSliderFunctions::getVal($frame_end, 'use_text_c') == 1)
            $frame_end['color'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_end, 'text_c', 'transparent'));

        if (RevSliderFunctions::getVal($frame_end, 'use_bg_c') == 1)
            $frame_end['bgcolor'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_end, 'bg_c', 'transparent'));
        ...
        $frame_end['sfxcolor'] = RevSliderTPColorpicker::get(RevSliderFunctions::getVal($frame_end, 'sfxcolor', RevSliderFunctions::getVal($layer, 'sfxcolor', '#ffffff'))); //fallback
                ...
                if (!empty($static_styles['color'])) {
                    if (is_object($static_styles['color'])) {
                        ...
                        foreach ($static_styles['color'] as $sk => $sv) {
                            $color_trans = RevSliderFunctions::getVal($def_val, 'color-transparency', false);
                            $static_styles['color']->$sk = RevSliderTPColorpicker::convert($sv, $color_trans);
                        }
        ...
        $calc_bg_color = RevSliderTPColorpicker::convert($bg_color, $bg_trans);
        $calc_dbc_color = RevSliderTPColorpicker::convert($dbc, $dbt);

        if (strpos($calc_bg_color, "gradient") !== false || strpos($calc_dbc_color, "gradient") !== false)
            $st_idle['background'] = array(RevSliderTPColorpicker::convert($bg_color, $bg_trans), RevSliderTPColorpicker::convert($dbc, $dbt)); //'ALWAYS'

        else
            $st_idle['background-color'] = array(RevSliderTPColorpicker::convert($bg_color, $bg_trans), RevSliderTPColorpicker::convert($dbc, $dbt)); //'ALWAYS'
        ...
        $dbot = intval($dbot);
        $st_idle['border-color'] = array(RevSliderTPColorpicker::convert($border_color, $border_trans), RevSliderTPColorpicker::convert($dboc, $dbot)); //'ALWAYS'
        //}
        ...
        if ($is_hover_active) {
            ...
            if ($font_color !== 'transparent') {
                ...
                $st_hover['c'] = array(RevSliderTPColorpicker::convert($font_color, $font_trans), 'ALWAYS');
            } else {
            ...
            $st_hover['bg'] = array(RevSliderTPColorpicker::convert($bg_color, $bg_trans), 'ALWAYS');
            //}
            ...
            $st_hover['bc'] = array(RevSliderTPColorpicker::convert($border_color, $border_trans), 'ALWAYS');
            //}
