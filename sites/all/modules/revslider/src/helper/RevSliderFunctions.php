...
class RevSliderFunctions
{
    ...
    {
        $params = array();
        $params = array_merge($params, $add_arams);
        $params = array_merge($params, array(
            'view' => $viewName
        ));
        $admin_path = RevSliderRoute::route('admin');
        return self::create_url($admin_path, $params);
    }
    ...
    public static function ajax_url(array $args = array())
    {
        ...
        $ajax_path = RevSliderRoute::route('ajax');
        return self::create_url($ajax_path, $params);
    }
    ...
    {
        return self::sanitize_title($str);
    }
    ...
    public static function check_file_in_zip($d_path, $image, $alias, &$alreadyImported, $add_path = false)
    {
        ...
        if (trim($image) !== '') {
            if (strpos($image, 'http') !== false) {
                ...
                if (!$zimage) {
                    $zimage = RevSliderFile::fileExists(RevSliderFunctions::merge_path( $d_path , 'images/' ,$image));
                    $strip = true;
                }
                if (!$zimage) {
                    ...
                    if (!isset($alreadyImported['images/' . $image])) {
                        ...
                        $importImage = RevSliderFunctionsWP::import_media(RevSliderFunctions::merge_path($d_path , $uimg), $alias . '/');
    ...
    {
        $file = (isset($_FILES[$field])) ? $_FILES[$field] : false;
        return $file;
    }
    ...
    {
        $result = isset($_REQUEST[$var_name]) ? $_REQUEST[$var_name] : $default;
        return $result;
    }

    public static function getPostGetVar($key, $defaultValue = "")
    {
        return self::getRequestVariable($key, $defaultValue);
    }
    ...
    public static function asset_dir($path)
    {
        ...
        return self::merge_path($dir, '/assets/', $path);
    }
    ...
    public static function get_svg_sets_url()
    {
        ...
        $path = self::asset_dir('public/assets/svg/');
        $url = self::asset('public/assets/svg/');
...
//        return apply_filters( 'wp_get_attachment_image_src', $image, $attachment_id, $size, $icon );
    ...
    {
        $full_url = url('', array('absolute' => TRUE));
        return $full_url;
    }
    ...
    public static function current_link()
    {
        ...
        $full_url = self::full_url($request_path);
        return $full_url;
    }
    ...
    public static function validate_dir($dir, $remove_end_slash = true)
    {
        ...
        $is_drupal_path = false;
        if (strpos($dir, $public_prefix) === 0) {
            $dir = str_replace($public_prefix, '', $dir);
            $is_drupal_path = true;
        }
        $dir_result = preg_replace('/[\/]{1,}/i', '/', $dir);
        if ($remove_end_slash && substr($dir_result, -1) === '/')
            $result = substr($dir_result, 0, -1);
        else
            $result = $dir_result;
        if ($is_drupal_path) {
            if (substr($result, 0, 1) == '/')
                $result = substr($result, 1);
            $result = $public_prefix . $result;
        }
        return $result;
    }
}